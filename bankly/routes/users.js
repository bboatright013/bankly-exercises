 ...
 *
 */
// removed requireAdmin, checks manually in the route, fixes user cant patch themself bug 
...
async function(req,res,next) {
  try {
    ...
    // }
    //MODIFIED if not admin and modifying admin status throw error FIXES BUG #4
   if (!req.curr_admin && req.body.admin) {
      throw new ExpressError('Only  that user or admin can edit a user.', 401);
    }

    //ADDED - Specify admin or user updates FIXES BUG #4
    if(req.curr_admin){
      let fields = { ...req.body };
...
router.delete('/:username', authUser, requireAdmin, 
async function(req,res,next) {
  try {
