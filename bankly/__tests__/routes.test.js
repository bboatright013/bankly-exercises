...
// TEST BUG #1
  test("should not allow a user to register with no data", async function() {
    const response = await request(app)
      .post("/auth/register")
      .send({
        username: "",
...
// TEST BUG #2
  test("should allow a correct username/password to log in as admin", async function() {
    const response = await request(app)
      .post("/auth/login")
      .send({
        username: "u3",
  ...
  // TEST BUG #3 part 1
  test("should reject if a user tries to login with no password", async function() {
    const response = await request(app)
      .post("/auth/login")
      .send({
        username: "u2",
    ...
    // TEST BUG #3 part 2
    test("should reject if a user tries to login with wrong password", async function() {
      const response = await request(app)
        .post("/auth/login")
        .send({
          username: "u2",
          password: "notu2spassword"
        });
      expect(response.statusCode).toBe(401);
    });
});

// Unhandled promise rejection with try catch block in GET /users
